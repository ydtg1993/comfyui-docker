...
try:
    ...
    # Backward/forward compatibility: some versions of Transformers don't expose
    # these private helpers in candidate_generator. Provide graceful fallbacks.
    from transformers.generation.candidate_generator import (
        AssistedCandidateGenerator,
        AssistedCandidateGeneratorDifferentTokenizers,
        CandidateGenerator,
    ...
    def _crop_past_key_values(past_key_values, max_length=None, keep_only_last_token=False):
        # No-op safe fallback; returns inputs unchanged
        return past_key_values

    def _prepare_attention_mask(model_input_names, inputs, model_kwargs, device=None, min_length=0):
        # Return existing attention_mask if available; else None
        return model_kwargs.get("attention_mask", None)
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # Be compatible with configs that do not define `forced_decoder_ids`
