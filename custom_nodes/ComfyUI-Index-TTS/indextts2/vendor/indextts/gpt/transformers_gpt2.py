...
try:
    ...
    # Fallback for Transformers versions where SequenceSummary is not exposed here
    class SequenceSummary(nn.Module):
        """Minimal compatible SequenceSummary.

        It selects the last token's hidden state and optionally applies a linear projection.
        This is sufficient for our inference path where classification heads are not used extensively.
        """
        ...
        def __init__(self, config):
            ...
            self.proj = None
            if hasattr(config, "num_labels") and hasattr(config, "hidden_size"):
                self.proj = nn.Linear(config.hidden_size, getattr(config, "num_labels", config.hidden_size))

        def forward(self, hidden_states, cls_index=None, **kwargs):
...
from .....compatibility_patch import find_pruneable_heads_and_indices, prune_conv1d_layer
...
from .....compatibility_patch import assert_device_map, get_device_map
